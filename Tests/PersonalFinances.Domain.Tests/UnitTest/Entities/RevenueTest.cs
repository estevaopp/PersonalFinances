...
namespace PersonalFinances.Domain.Tests.UnitTest.Entities
{
    public class RevenueTest
    {
        ...
        [InlineData("Freelance", 1, null, 100, "Salario no freelance")]
        public void CreateRevenue_ReturnSucess_CommandValid(string name, int revenueCategoryId, string date, decimal value, string description)
        {
            if(date == null)
            {
                var revenue = new Revenue(name, revenueCategoryId, null, value, description);
            ...
            {
                var revenue = new Revenue(name, revenueCategoryId, DateTime.Parse(date), value, description);
        ...
        public void CreateRevenue_ReturnBusinessException_CommandInvalid(string name, int revenueCategoryId, string date, decimal value, string description)
        {
            // Arrange
            // Act

            // Assert
            Assert.Throws<BusinessException>(() => new Revenue(name, revenueCategoryId, DateTime.Parse(date), value, description));
        }
        ...
        {
            var revenue = new Revenue("Salario", 5, new DateTime(2020,3,3), 1000, "Salario do emprego");
